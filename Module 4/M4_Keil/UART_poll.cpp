...
--			 UART_direct_hex_put - a routine that puts a hex byte directly to the UART port
--
--			NEW TO VERSION 2.0.1:
--				UART_low_nibble_direct_put() - puts the low nibble of a byte in hex directly
--  																			(no ram buffer) to the UART.
--			  UART_direct_word_hex_put() - puts a word in hex directly to the UART
                        ...
                        // received data or data to transmit
{
   ...
   if (TXIF)          //  Check if transmit buffer empty  
   {
      if ((tx_in_ptr != tx_out_ptr) && (display_mode != QUIET))
      {
         ...
         tx_in_progress = true;          /* flag needed to start up after idle */
      }
      else
      {
         tx_in_progress = false;             /* no more to send */
      }
  }                   
...
/*******************************************************************************/
/// @brief The function UART_direct_msg_put puts a null terminated string directly
...
/*****************************************************************************/
/// @brief  UART_direct_msg_put puts a null terminated string directly
/// (no ram buffer) to the UART in ASCII format.
/*****************************************************************************/
...
*/
/*****************************************************************************/
/// @brief UART_direct_msg_put puts a character directly
/// (no ram buffer) to the UART in ASCII format.
/*****************************************************************************/
void UART_direct_put(UCHAR c)
{
   TXREG = c;
   UART_TX_wait();
}
/*****************************************************************************/
/// \fn UART_get
/// @brief Gets the next byte available in the RX buffer.

/// UART_get gets the next byte if one is available from the receive
...
/*******************************************************************************/
/// @brief Checks if characters are sitting in the UART input buffer.

///
/// UART_input returns a 1 if 1 or more receive byte(s) is(are) 
...
{
   if( rx_in_ptr == rx_out_ptr )return(0);  
                              /* no characters in receive buffer */
      return(1);                        /* 1 or more receive characters ready */
}

/*****************************************************************************/
/// @brief UART_msg_put puts a null terminated string through the transmit
/// buffer to the UART port in ASCII format. (commented out)
/*****************************************************************************/
/*
...
/*****************************************************************************/
/// @brief Takes a single ASCII character and converts to hex. (commented out)
/*****************************************************************************/
/*
...
/*******************************************************************************
* The function UART_low_nibble_put puts the low nibble of a byte in hex directly
*  (no ram buffer) to the UART.
*******************************************************************************/
void UART_low_nibble_direct_put(UCHAR c)
{
...
/*****************************************************************************/
/// @brief UART_high_nibble_put puts the high nibble of a byte in h
/// UART port. (currently commented out)
/*****************************************************************************/
...
/*******************************************************************************
*! \brief UART_hex_put puts 1 byte in hex through the transmit buffer to 
* the UART port. (commented out)
*******************************************************************************/
/*
...
*/
/*****************************************************************************/
/// @brief UART_direct_hex_put puts 1 byte in hex directly (no ram buffer) 
/// to the UART.
/*****************************************************************************/
...
/*****************************************************************************/
/// @brief UART_direct_hex_put puts 4 bytes in hex directly (no ram buffer) 
/// to the UART.
/*****************************************************************************/
void UART_direct_word_hex_put(uint32_t word)
{
	UART_direct_hex_put((word>>24)&0xFF);
	UART_TX_wait();
	UART_direct_hex_put((word>>16)&0xFF);
	UART_TX_wait();
	UART_direct_hex_put((word>>8)&0xFF);
	UART_TX_wait();
	UART_direct_hex_put(word&0xFF);
	UART_TX_wait();
}
/*******************************************************************************/
/// @brief The function UART_direct_hex_int_put removes the leading zeroes and adds
/// a decimal point. Best used in conjunction with hex2hexInt
/******************************************************************************/
void UART_direct_hex_int_put(uint32_t word, uint8_t deci)
{
	bool zeros = true;
	int8_t i = 7; //must be signed because of wrap-around
	for(i=7;i>=0;i--)
	{
		if(zeros && ((word>>(i*4))&0xF)==0);
		else
		{
			zeros = false;
			UART_low_nibble_direct_put((word>>i*4)&0xF);
			if(i == deci & deci>0) UART_direct_put('.');	
		}
  }
}
