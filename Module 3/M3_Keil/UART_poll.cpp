 ...
 ***********************************/
 ...
 UCHAR error_count = 0; //<! variable for error counter
/*****************************************************************************/
///  \fn void serial(void) 
                        ...
                        // received data or data to transmit
{
   ...
   else              // else if no frame error,
   {
      ...
      {             // Read byte to enable reception of more bytes
                    // For PIC, deb
				//RCIF automatically cleared when RCREG is read
...
/// (no ram buffer) to the UART in ASCII format.
/*****************************************************************************/
void UART_direct_put(UCHAR chr)
{ 
      TXREG = chr; 
		  while( TXIF == 0 )
   {
...
/*****************************************************************************/
/// \fn UART_put(UCHAR)
/// @brief Puts a byte into the transmit buffer
///
/// Puts a byte, to the transmit buffer at the location 
...
/*****************************************************************************/
/// @brief Takes a single ASCII character and converts to hex.
/*****************************************************************************/
...
/*****************************************************************************/
/// @brief UART_low_nibble_put puts the low nibble of a byte in hex directly
/// (no ram buffer) to the UART.
/*****************************************************************************/
