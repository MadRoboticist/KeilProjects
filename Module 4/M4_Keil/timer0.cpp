...
void timer0(void)
 {
	 static   uint16_t display_led = 0; // start counter for red led
...
//    A. Display timer and flag
      display_timer--; // decrement display timer every 6.4 ms.  Total time is      
		 ...
		 display_led++; // increments led timer every 6.4 ms. 
		 /***************************************************************
		  * step counter from 0 to 155 for a total of 156 steps
		  * to create a 1 second timer. (156*6.4ms = 0.9984 sec).
		  * then reset the counter and start over.
		  **************************************************************/
		 if(display_led == 155)	
		 { display_led = 0;
		 }
...
//   Generate Outputs  ************************************
    ...
    // Create an 0.5 second RED LED heartbeat here. 
			...
			/*if counter is equal to 0 then trigger.*/
			if(display_led == 0){
			redLED = !redLED;
			}
   }   // end 6.4 ms group B
